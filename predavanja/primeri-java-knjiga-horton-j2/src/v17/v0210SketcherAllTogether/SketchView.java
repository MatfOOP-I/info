	...
	implements Observer, Constants, ActionListener, Printable, Pageable
{
	...
	public PageFormat getPageFormat(int pageIndex)
	{
		...
		{ // We are printing a sketch so decide
	...
	class MouseHandler extends MouseInputAdapter
	{
		...
		public void mouseDragged(MouseEvent e)
		{
			...
			else if (button1Down && mode == ROTATE && selectedElement != null)
			{
				...
				selectedElement.rotate(
					getAngle(selectedElement.getPosition(), start, last));
				selectedElement.draw(g2D); // Draw in its new position
				start = last; // Make start current point
			}
		}
		...
		public void mouseReleased(MouseEvent e)
		{
			if (e.isPopupTrigger())
			{
				...
				if (highlightElement == null)
					theApp.getWindow().getPopup().show(
	...
	JOptionPane.PLAIN_MESSAGE); // No icon
				...
				{ // If we have text        
					...
					Font font = theApp.getWindow().getCurrentFont();
						tempElement = new Element.Text(font, // The font
		text, // The text
